...
public class DSPN implements DocGhiFile{
    ...
    DSPN(ArrayList<PHIEUNHAP> dspn, long tongSoTienNhapHang) {
        this.dspn = new ArrayList<>();
    ...
    public void taoDSPN(KHO kho, DSNCC dsncc, int manv){
    ...
    //TAO PHIEU MOI
    public void taoPN(String ma, KHO kho, DSNCC dsncc, int manv){
        PHIEUNHAP pn = new PHIEUNHAP();
        pn.khoiTaoPN(ma, kho, dsncc, manv);
        dspn.add(pn);
        tinhTongTienDSPN();
        ghiFile();
    }
    ...
    */
    ...
    public int ktPNTonTaiTheoMaNCC(String ma){
        int length = dspn.size();
        int count = 0;
        for(int i=0; i<length; i++){
            if(dspn.get(i).getSupplier().equals(ma)){
    ...
    */
    //THEM PHIEU MOI
    public void themPN(KHO kho, DSNCC dsncc, int manv){
        ...
        taoPN(ma, kho, dsncc, manv);
    }
    //THEM MAT HANG VAO PHIEU DA TIN TAI HOAC TAO PHIEU MOI NEU KO TON TAI PHIEU
    public void themVaoPNDaTonTai(KHO  kho, DSNCC dsncc, int manv){
    ...
    public void xoaPNTheoMaNCC(DSNCC dsncc){
        ...
        outer :
        while(true){
            int count = ktPNTonTaiTheoMaNCC(ma);
            if(count == 0){
    ...
    public void menuThem(KHO kho, DSNCC dsncc, int manv){
        them: 
        while(true){
            ...
            switch(mode2){
                case 1: {
                    themPN(kho, dsncc, manv);
                }
                    break;
                case 2: {
                    themVaoPNDaTonTai(kho, dsncc, manv);
                }
                    break;
                case 0:
                    break them;
    ...
    public void menuPn(KHO kho, DSNCC dsncc, int manv){
    outer:
        while(true){
            ...
            switch(n){
                ...
                case 3: {
                    if(getDspn().isEmpty()){
                        System.out.println("Khong co phieu nhap!");
                        System.out.println("Ban co muon tao 1 danh sach phieu nhap moi khong?");
                        System.out.print("Y / N : ");
                        String confirm = inp.nextLine();
                        while(true){
                            if(confirm.equalsIgnoreCase("y") || confirm.equalsIgnoreCase("Y")){
                                taoDSPN(kho, dsncc, manv);
                                break inner;
                            }
